<%@ page language="java" import="edu.thn.ciom.pojo.Jcbiaoti"  pageEncoding="utf-8"%>
...
var url;
	...
	function qiyongJcbiaoti(){
		...
		$.messager.confirm("系统提示","您确认要启用这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangJcbiaoti",{delIds:ids,jcbiaotiType1:0},function(result){
					if(result.success){
	...
	function jinyongJcbiaoti(){
		...
		$.messager.confirm("系统提示","您确认要启用这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangJcbiaoti",{delIds:ids,jcbiaotiType1:1},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功启用<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
