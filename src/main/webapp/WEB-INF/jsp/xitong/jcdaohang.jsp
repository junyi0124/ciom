<%@ page language="java" import="edu.thn.ciom.pojo.Jcdaohang"  pageEncoding="utf-8"%>
...
var url;
	...
	function qiyongJcdaohang(){
		...
		var ids=strIds.join(",");
		$.messager.confirm("系统提示","您确认要启用这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangJcdaohang",{delIds:ids,jcdaohangType1:0},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功启用<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
						$.messager.alert('系统提示','<font color=red>'+selectedRows[result.errorIndex].jcdaohangName+'</font>'+result.errorMsg);
					}
	...
	function jinyongJcdaohang(){
		...
		var ids=strIds.join(",");
		$.messager.confirm("系统提示","您确认要禁用这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../duoxiangJcdaohang",{delIds:ids,jcdaohangType1:1},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功禁用<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
						$.messager.alert('系统提示','<font color=red>'+selectedRows[result.errorIndex].jcdaohangName+'</font>'+result.errorMsg);
					}
